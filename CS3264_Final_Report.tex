...
We also removed zero bits (chemical structures universally absent across our dataset),which reduces the effective dimensionality is reduced from 166 to x.  It is necessary because zero bits do not contribute to the model's decision and may introduce noise, while also improving computational efficiency. Removing zero bits can also enhance the interpretability of the model by focusing on the relevant chemical features. For instance, Key 2 is interpreted as `Contains an atom with atomic number $\geq$ 104 (e.g. Rutherfordium)', which is very unlikely to be present in any drug molecule, and thus can be safely removed without losing any predictive information.
% TODO: Perform data cleaning such that zero bits are removed and report the final dimension of the input vector after removing zero bits, which bits are removed and the interpretation of those bits.
...
\section{Results}
% TODO: Add experimental details and results.
\subsection{Prediciive performance}
