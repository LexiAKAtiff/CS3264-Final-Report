...
\
\newline
Each drug is represented as a 166-dimensional MACCS Keys (Molecular ACCess System) binary fingerprint \cite{rdkit}, encoding the presence or absence of predefined chemical substructures (e.g., Key 158 = carbon–nitrogen single bond). MACCS Keys are widely adopted in pharmaceutical research \cite{xie2020improvement,durant2002reoptimization} and, unlike Morgan fingerprints, each bit maps to a named, chemically interpretable substructure, directly supporting our interpretability analysis. After removing zero-variance bits (universally absent across our dataset), the effective dimensionality is reduced from 166 to 162. The model learns to predict the interaction label based on the combined features of the two drugs.
...
Oringinal MACCS Keys: \url{https://github.com/rdkit/rdkit/blob/master/rdkit/Chem/MACCSkeys.py}
Human-readable MACCS Keys: \url{}
\end{document}
