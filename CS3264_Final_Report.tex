...
\begin{equation}
    ...
    \label{eq:gini}
\end{equation}
For binary classification, $G(S) = 2\hat{p}(1-\hat{p})$, where $\hat{p}$ is the fraction of positive samples in $S$. The key randomisation that distinguishes Random Forest from bagged trees is that the search in \eqref{eq:cart_split} is restricted to a \emph{randomly sampled} feature subset $\mathcal{M}_v \subseteq \{1, \ldots, p\}$ of size $m \ll p$ at each node, typically $m = \lfloor\sqrt{p}\rfloor$ for classification tasks\cite{Breiman2001}. This further decorrelates the trees and improves generalisation.
...
\begin{equation}
    \tilde{y} = \mathbf{1}[\hat{y} > \delta].
\end{equation}
Where $\mathbf{1}[\hat{y} > \delta]$ is the indicator function that outputs 1 if $\hat{y} > \delta$ and 0 otherwise. The threshold $\delta$ can be tuned based on the desired balance between sensitivity and specificity.

The variance of $F$ relative to a single tree can be expressed as
\begin{equation}
    \mathrm{Var}[F] = \rho \sigma^2 + \frac{1-\rho}{T}\sigma^2,
    \label{eq:rf_var}
\end{equation}
where $\sigma^2$ is the per-tree variance and $\rho$ is the pairwise correlation between trees \cite{Breiman2001}. Equation \ref{eq:rf_var} shows that as $T \to \infty$, the irreducible variance $\rho\sigma^2$ depends only on $\rho$: the random feature selection in \eqref{eq:cart_split} explicitly suppresses $\rho$, yielding better generalisation than a single tree without increasing bias.
...
\textit{(iii) High-dimensional sparse inputs.} With $p = 162$ features, many bits are inactive (value 0) for any given drug pair. Because CART only searches over $m = \lfloor\sqrt{162}\rfloor = 12$ features per node, each tree focuses on a small, potentially informative subset, making the method empirically robust to the many irrelevant bits common in fingerprint representations. \cite{Svetnik2003}.
 
\textit{(iv) Robustness via bagging.} Bootstrapping introduces diversity among trees, which by \eqref{eq:rf_var} reduces prediction variance. This is particularly valuable in a medical setting where stability of predictions across slight perturbations of the input is a prerequisite for clinical trust. \cite{Rudin2019}.
...
\paragraph{Algorithm.}
Algorithm~\ref{alg:trdp} states the complete procedure.
 
\begin{algorithm}[t]
\caption{Tree-Ranked Decision Path (TRDP) Output}
\label{alg:trdp}
\begin{algorithmic}[1]
\REQUIRE Fitted forest $\mathcal{F} = \{\mathcal{T}_t\}_{t=1}^{T}$,
         test sample $\mathbf{x}$, number of trees to report $K$
\ENSURE  Ranked list of $K$ full decision paths with confidence scores
\STATE Compute per-tree leaf confidence $\hat{p}_t(\mathbf{x})$
       for all $t \in \{1,\ldots,T\}$
\STATE $\mathcal{T}^{+} \leftarrow
       \{t : \hat{p}_t(\mathbf{x}) > 0.5\}$
       \hfill \COMMENT{Eq.\ \ref{eq:pos_trees}}
\STATE Sort $\mathcal{T}^{+}$ by $\psi(t,\mathbf{x}) = \hat{p}_t(\mathbf{x})$
       in descending order
       \hfill \COMMENT{Eq.\ \ref{eq:tree_score}}
\STATE $\mathcal{T}^{*} \leftarrow$ top-$K$ trees from sorted
       $\mathcal{T}^{+}$
       \hfill \COMMENT{Eq.\ \ref{eq:top_k}}
\FOR{each tree $t \in \mathcal{T}^{*}$ (in rank order)}
    \STATE Retrieve path
           $\Pi_t(\mathbf{x}) = (c_1^{(t)}, \ldots, c_{d_t}^{(t)})$
           via \texttt{decision\_path()}
    \FOR{each node $l = 1$ to $d_t$}
        \STATE Map $(j_l^{(t)}, \tau_l^{(t)}, \delta_l^{(t)})$ to
               MACCS key name and condition string
               \hfill \COMMENT{Eqs.\ \ref{eq:sem_one}--\ref{eq:sem_none}}
    \ENDFOR
    \STATE Output ranked path with header
           $\psi(t, \mathbf{x})$
\ENDFOR
\RETURN $K$ annotated conjunction rules in confidence order
\end{algorithmic}
\end{algorithm}
\subsubsection{MLP}
\ 
