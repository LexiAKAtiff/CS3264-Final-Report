...
Considering patient condition, we use \textbf{FDA FARES} dataset, which contain reports of patients having adverse reactions after taking certain combination of drugs with their personal information about age, gender and etc.with the patients' consent.
...
During data preprocessing, we removed MACCS bits with zero across all drugs in the dataset, which also means the bits representing chemicals that are universally absent. These chemical strucutres are not conventionally used in drugs and their removal reduces noise in the input representation. After this filtering step, the effective fingerprint dimensionality is reduced from 166 to 162 bits per drug.
...
We can express a simplified example of how the MLP learns to capture complex chemical interactions as the following.
...
\
\newline
Here we adopt a novel strategy: we
...
conjunction of conditions and istherefore directly interpretable by a clinician without further aggregation.
...
contribution of feature $j$ across all feature orderings. However, the tree-rank path better captures the reaction that involves several \textbf{groups} of chemicals. On the other hand, SHAP's ranking of individual features may fail to capture the interaction between a group of chemicals that are present in the same drug pair, which is a common mechanism of drug interactions.
...
p
    & Feature dimensionality; $p = 162$ after MACCS preprocessing \\
\mathbf{x}_i
